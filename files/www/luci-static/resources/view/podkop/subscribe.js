...
function createConfigListUI(configs, listId, section_id, isUrltest, isSelector) {
  ...
  configs.forEach(function (config, index) {
    ...
    if (isUrltest) {
      ...
      configItem.onclick = createUrlClickHandler(config, configItem, configList, section_id, isXhttp, index);
    }
...
function createUrlClickHandler(config, configItem, configList, section_id, isXhttp, configIndex) {
  return function (e) {
    e.stopPropagation();
    ...
    // Auto-update the subscribe_selected_index field so the chosen index is saved
    var indexInput =
      document.getElementById("widget.cbid.podkop." + section_id + ".subscribe_selected_index") ||
      document.querySelector('input[id*="' + section_id + '"][id*="subscribe_selected_index"]');
    if (indexInput) {
      indexInput.value = String(configIndex !== undefined ? configIndex : 0);
...
function enhanceSectionWithSubscribe(section) {
  ...
  // Auto-update interval (only for single-config "url" mode)
  o = section.option(
    form.ListValue,
    "subscribe_interval",
  ...
  o.value("24", _("Каждые 24 часа"));
  o.value("48", _("Каждые 48 часов"));
  o.value("72", _("Каждые 72 часа"));
  o.default = "0";
  o.rmempty = true;

  // Index of the config to apply during auto-update (0-based)
  o = section.option(
    form.Value,
    "subscribe_selected_index",
